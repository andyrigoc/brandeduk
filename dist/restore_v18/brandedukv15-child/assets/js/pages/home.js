...
function renderProducts(productsToRender = PRODUCTS_DB) {
    ...
    productsToRender.forEach((product, index) => {
        ...
        const priceRangeText = `£${minPrice.toFixed(2)} - £${maxPrice.toFixed(2)}`;
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                <div class="product-price">${priceRangeText}</div>
...
function applyFilters(options = {}) {
    ...
    document.querySelectorAll('input[name="manufacturer"]:checked').forEach(cb => {
        filters.manufacturers.push(cb.value);
    });
    document.querySelectorAll('input[name="process"]:checked').forEach(cb => {
        filters.processes.push(cb.value);
    });
