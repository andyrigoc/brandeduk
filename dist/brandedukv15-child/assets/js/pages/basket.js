    ...
    tbody.innerHTML = `
        <tr>
            <td colspan="6" style="text-align: center; padding: 3rem;">
                ...
                <a href="home-pc.html" class="btn-primary" style="display: inline-block; padding: 0.75rem 1.5rem; background-color: #0033a0; color: white; text-decoration: none; border-radius: 6px;">Start Shopping</a>
            </td>
        </tr>
    `;
}
...
function renderBasketItems(productData, positionCustomizations, selectedPositions) {
    ...
    row.innerHTML = `
        ...
        <td class="price-cell">£${totalGarmentCost.toFixed(2)}</td>
        <td>
...
function calculateBreakdown() {
    ...
    if (!productData) {
        document.getElementById('garmentCosts').textContent = '£0.00';
        document.getElementById('applicationCosts').textContent = '£0.00';
        document.getElementById('logoSetupCosts').textContent = '£0.00';
        document.getElementById('deliveryCosts').textContent = '£0.00';
        document.getElementById('vatCosts').textContent = '£0.00';
    ...
    document.getElementById('garmentCosts').textContent = `£${garmentTotal.toFixed(2)}`;
    
    // Application costs (sum of all positions)
    ...
    positionCustomizations.forEach((customization) => {
        ...
        if (positionPrices[positionName]) {
            ...
            applicationTotal += price * totalQuantity;
        }
    });
    
    document.getElementById('applicationCosts').textContent = `£${applicationTotal.toFixed(2)}`;
    
    // Logo setup cost
    const hasLogo = positionCustomizations.some(c => c.uploadedLogo);
    const logoSetupCost = hasLogo ? 12.00 : 0;
    document.getElementById('logoSetupCosts').textContent = `£${logoSetupCost.toFixed(2)}`;
    
    // Delivery
    const deliveryCost = 0; // Free delivery
    document.getElementById('deliveryCosts').textContent = `£${deliveryCost.toFixed(2)}`;
    ...
    document.getElementById('vatCosts').textContent = `£${vatCost.toFixed(2)}`;
