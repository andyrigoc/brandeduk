...
function initHeroBanners() {
    ...
    const brandedIndex = bannersArr.findIndex(b => b.id === 'heroBanner4');
    let currentBanner = brandedIndex >= 0 ? brandedIndex : 0;
    let autoRotateTimeout;

    function getDurationMsForIndex(index) {
        const el = bannersArr[index];
        if (!el) return 10000;
        return el.id === 'heroBanner4' ? 20000 : 10000;
    }
    
    // Initialize: branded banner active by default
    bannersArr.forEach((banner, i) => {
        if (i === currentBanner) {
    ...
    dotsArr.forEach((dot, i) => {
        dot.classList.toggle('banner-dot--active', i === currentBanner);
    });
    ...
    function switchBanner(index) {
        ...
        const currentEl = bannersArr[currentBanner];
        const nextEl = bannersArr[index];
    ...
    function scheduleNextRotation() {
        ...
        autoRotateTimeout = setTimeout(() => {
            const nextIndex = (currentBanner + 1) % bannersArr.length;
            switchBanner(nextIndex);
        }, getDurationMsForIndex(currentBanner));
    }
    ...
    // Start auto-rotate (per-banner durations)
    scheduleNextRotation();
}

// ============================================
// HERO BOUNCE TEXT - Split chars (Desktop)
// ============================================
function initBounceText() {
    ...
    bounceEls.forEach((el) => {
        if (el.dataset.bounceProcessed === 'true') return;

        const text = el.getAttribute('data-bounce-text') || el.textContent || '';
        el.textContent = '';
        ...
        el.dataset.bounceProcessed = 'true';
    });
}
