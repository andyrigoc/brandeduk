...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.position-card').forEach(card => {
        // The inline Step 3 positions grid on product-detail is controlled by
        // `customize-positions-inline.js` (it handles method badges + reset behavior).
        // Avoid double-binding conflicting click handlers here.
        if (card.closest('#step3PositionsSection')) {
            return;
        }
        ...
        card.addEventListener('click', (e) => {
            // Don't toggle selection when interacting with controls inside the card.
            if (e.target.closest('button, a, input, label, .price-badge, .delete-logo-btn')) {
                return;
            }
