...
function initPositionSelection() {
    ...
    cards.forEach(card => {
        ...
        card.addEventListener('click', (e) => {
            ...
            // Let direct checkbox clicks behave normally (change handler will run)
            if (e.target.closest('input')) return;

            // Prevent label default behavior toggling the checkbox behind our back
            e.preventDefault();
            
            // Don't toggle if clicking on the customization pill (informational only)
            if (e.target.closest('.customization-pill')) return;

            // If a method was chosen but the user hasn't uploaded/applied any customization yet,
            ...
            const hasCustomization = Boolean(positionCustomizationsMap[position]) || card.classList.contains('customized');
        ...
        checkbox.addEventListener('change', () => {
            ...
            if (checkbox.checked) {
                // Enforce explicit method choice (no default). If user toggles the checkbox
                // via the label/checkbox without picking a method, revert and prompt.
                if (!positionMethods[position]) {
                    checkbox.checked = false;
                ...
                if (existingIndex === -1) {
                    selectedPositions.push({
                        ...
                        method: positionMethods[position] || null
                    });
                }
                refreshCardState(position);
            } else {
